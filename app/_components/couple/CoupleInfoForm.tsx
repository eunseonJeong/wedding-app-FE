...
import { ChildRank, ChildRankValue, BusType, BusData } from "@/app/utils/enums";
...
interface CoupleInfoFormProps {
  coupleInfo: ServerCoupleInfo | null;
}

const defaultValues: CoupleFormData = {
  man: {
    name: "",
    father: "",
    mother: "",
...
const CoupleInfoForm = ({ coupleInfo }: CoupleInfoFormProps) => {
  const router = useRouter();
  const [formData, setFormData] = useState<CoupleFormData>(defaultValues);

  // coupleInfo가 변경될 때 formData 업데이트
  useEffect(() => {
    if (coupleInfo) {
      const transformedImages: FormImage[] = coupleInfo.images.map((img) => ({
        file: null,
        preview: img.url,
        isMain: img.isMain,
      }));
  ...
  const handleChange = (
    ...
    if (section) {
      setFormData((prev) => ({
        ...prev,
        [section]: {
          ...(prev[section] as Record<string, never>),
          [field]: value,
        },
      }));
    } else {
