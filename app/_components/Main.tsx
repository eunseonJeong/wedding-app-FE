...
import { Header, Footer } from "@/app/_components/common";
...
export const Main = ({ data }: { data: WeddingData }) => {
  ...
  // 페이지 타이틀과 설명에 사용할 정보
  const brideFirstName = getFirstName(weddingData.woman?.name);
  const groomFirstName = getFirstName(weddingData.man?.name);
  ...
  const busData = weddingData?.bus || {};

  useEffect(() => {
    const data = async () => {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/couple`,
      );
          ...
          <div className="flex justify-between w-full mb-12">
            <div className="text-center">
              ...
              <p className="text-sm font-medium mb-1 text-wedding-text dark:text-wedding-darkText">
                {weddingData.man.name}
              </p>
              <p className="text-xs text-wedding-muted dark:text-wedding-darkMuted">
                {weddingData.man.father} · {weddingData.man.mother}의{" "}
                {weddingData.man.rank}
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-sm font-medium mb-1 text-wedding-text dark:text-wedding-darkText">
                {weddingData.woman.name}
              </p>
              <p className="text-xs text-wedding-muted dark:text-wedding-darkMuted">
                {weddingData.woman.father} · {weddingData.woman.mother}의{" "}
                {weddingData.woman.rank}
              </p>
            </div>
          </div>
